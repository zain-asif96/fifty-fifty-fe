...
const addCurrency = async () => {
    ...
    let payload = {
        ...currency,
        special_rate: Number(currency?.special_rate),
        rate: Number(currency?.rate)

    }

    try {
        const res = await axios.post(BASE_URL + '/currencies', payload, {
        ...
        if (res.data.id || res.data.status === 'CREATED') {
            ...
            endAdd();
            emit('currencyAdded', res.data?.data);
        }
    } catch (errors) {
    ...
    <div>
        ...
        <form v-show="formOpened" class="border-gray-400 border rounded-lg p-6 mb-8">
            <div class="grid gap-6 mb-10 md:grid-cols-2">
                <TextInput v-model="currency.code" :errors="api.errors.value?.code" label="Currency code" placeholder="USD"
                    required title="code" />

                <TextInput v-model="currency.name" :errors="api.errors.value?.name" label="Currency Name"
                    placeholder="American Dollar" required title="name" />

                <TextInput v-model="currency.rate" :errors="api.errors.value?.rate" label="Rate" placeholder="Rate" required
                    type="number" title="name" />

                <TextInput v-model="currency.special_rate" :errors="api.errors.value?.special_rate" label="Special Rate"
                    placeholder="Special Rate" required type="number" title="special_rate" />

                <SelectInput v-model="currency.rate_source" :errors="api.errors.value?.rate_source" :options="sourceOptions"
                    label="Rate Source" required placeholder=" Select " title="currency" type="text" />
            </div>

            <div class="flex gap-4 items-center">
                <button @click="addCurrency" type="button"
